...
**总体判断**：7/14 呈现 **「CLI 运行时硬ening 日 + 可观测自托管补齐 + Sol 数学 PR 进入第二传播波」**——Claude Code **三连发** 把 **worktree 泄露** 与 **间接注入** 推成 **默认可利用面修复**；Langfuse **Monitors 下放到 self-hosted** 缩小 cloud/自建差距；OpenAI **CDC 证明** 在 **7/14** 被 **Scientific American** 定性为 **benchmark 叙事** 而非 **已验收定理**。
