...
**问**：在 Skills/Tools 执行的同时，知识库（RAG）如何并行参与？子流程包含哪些步骤？

**答**：

- **触发时机**：意图识别与路由阶段即可启动，与 Skill 匹配、Tool 调用**并行**，不阻塞 MCP 工具执行。
- **子流程三步**：
  1. **查询重写**：将用户自然语言转为更适合检索的查询（消歧、补全实体）。
  2. **向量数据库检索**：Embedding 相似度召回相关文档片段。
  3. **上下文增强**：将检索结果整理为可注入 Prompt 的上下文块。
- **结果汇入**：增强后的上下文与 Tool 执行结果（如 `execute_sql` 数据集）在**结果整合**阶段一并进入模型上下文。
- **价值**：Tool 提供动态/实时数据，RAG 提供静态参考（数据字典、业务规则、历史文档），二者互补后生成更准确的最终答案。
