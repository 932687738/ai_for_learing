<!-- 模块：RAG Advisor | 最后更新于 2026-05-28 -->

# RAG Advisor

> QuestionAnswerAdvisor 与回答质量检测等横切能力。

## 目录

- [QuestionAnswerAdvisor 检索结果缓存](#questionansweradvisor-检索结果缓存)
- [ResponseValidationAdvisor 回答质量检测](#responsevalidationadvisor-回答质量检测)
- [QuestionAnswerAdvisor 工作流程](#questionansweradvisor-工作流程)

---
## QuestionAnswerAdvisor 检索结果缓存

> **模块**：RAG Advisor | **标签**：RAG Advisor | **更新**：2026-05-28
...
**答**：缓存 Key**：归一化查询文本的 MD5，或查询向量的哈希。；避免污染**：设置合理 TTL（如 5–30 分钟）；对含「今天」「昨天」等时效性查询动态禁用缓存；使用 `@Cacheable` 的 `condition` 属性。。
...
**答**：**检测维度**：事实准确性、安全性、格式合规、相关性、逻辑一致性、来源可溯。 **实现方式**： 1. **规则校验**：JSON 解析、正则、关键词黑名单等确定性检查。 2. **LLM-as-Judge**：二次调用模型审查首答，错误则纠正或拒绝。 3. **Advisor 集成**：实现 `CallAdvisor`，在 `chain.nextCall()` 之后校验响应，不通过则抛异常或触发重试。
...
**答**：1. **Before 阶段**：调用 `VectorStore.similaritySearch` 检索相关文档。 2. **调用大模型前**：将检索到的 `Document.content`（文本片段）拼入 Prompt 作为上下文——传递的是**文本**，不是向量。
